---
name: replicate
description: Replicate.ai model hosting — run open-source and commercial ML models via API. Use for FLUX, SDXL, video models, and any model hosted at replicate.com/models.
when_to_use: Trigger on replicate, replicate.ai, REPLICATE_API_TOKEN, run model replicate, FLUX replicate, SDXL replicate.
allowed-tools: Bash
---
...
```bash
bash /Users/vasanth/Code/skills/replicate/sync-models.sh
```
...
tail -n +2 /Users/vasanth/Code/skills/replicate/models.jsonl | python3 -c "
import sys,json
kw = 'flux'  # change this
...
**Key source:** `~/.gsai/secrets.env` → `REPLICATE_API_TOKEN` — ⚠️ **Not provisioned yet.** Get at replicate.com/account/api-tokens, then uncomment the line in `secrets.env`.
...
```bash
RESULT=$(curl -s -X POST "https://api.replicate.com/v1/predictions" \
...
Find versions at `https://replicate.com/{owner}/{name}/versions`
...
```bash
curl -s "https://api.replicate.com/v1/models/$OWNER/$NAME/versions" \
  -H "Authorization: Token $REPLICATE_API_TOKEN" \
...
- Cost is per-second of compute — video models can be expensive; check pricing at replicate.com/pricing
