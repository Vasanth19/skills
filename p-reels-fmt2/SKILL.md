...
(`p-bottom-avatar-pip` and the upload variant `p-bottom-avatar-pip-upload`). Those use **ffmpeg
...
- Decide: does this format SUBSUME the existing `p-bottom-avatar-pip` recipes, or wrap/delegate
...
- Confirm whether the default is HeyGen-rendered avatar (`p-bottom-avatar-pip`) or user-uploaded
  talking head (`p-bottom-avatar-pip-upload`) — they have different first steps.
