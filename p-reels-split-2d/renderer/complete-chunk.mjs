/**
 * complete-chunk.mjs — renders missing frames for a chunk
 * Call from /Users/vasanth/Code/skills/p-reels-split-2d/renderer/
 * so that the canvas module can be found
 */
...
// Args: beats.json startFrame endFrame pngDir fps font
const [,, beatsPath, startFrameStr, endFrameStr, pngDir, fpsStr, fontPath] = process.argv;
const startFrame = parseInt(startFrameStr);
const endFrame = parseInt(endFrameStr);
const FPS = parseInt(fpsStr || "30");
const W = 1080, H = 960;
const SAFE_FONT = "'Proxima Nova', sans-serif";
...
// Load illustrations from current dir
const { drawAgentGrid, drawStatCard, drawParallelLanes, drawOrgChart, drawIdemock } =
  await import("./illustrations.mjs");
const illustrations = { drawAgentGrid, drawStatCard, drawParallelLanes, drawOrgChart, drawIdemock };

// Load beats (already shifted)
...
function beatState(beat, t) {
  ...
  if (isOut && outProg > 0) switch (beat.outAnim) {
  ...
  let idleScale = 1;
  if (!isIn && !isOut && beat.idle) switch (beat.idle) {
...
function drawDropShadow(ctx, fn) {
  for (const { blur, offsetY, alpha } of [{blur:24,offsetY:12,alpha:0.12},{blur:12,offsetY:6,alpha:0.18},{blur:6,offsetY:3,alpha:0.22}]) {
    ctx.save(); ctx.shadowColor=`rgba(0,0,0,${alpha})`; ctx.shadowBlur=blur; ctx.shadowOffsetX=0; ctx.shadowOffsetY=offsetY; fn(ctx); ctx.restore();
  }
}
...
function strokeLiquidOutline(ctx, cx, cy, pw, ph, r, palette, t) {
  ...
  const x=cx-pw/2, y=cy-ph/2, totalPerim=2*(pw+ph);
  const points=[];
  for (let i=0;i<N;i++) {
...
function drawPrimitive(ctx, beat, t, state) {
  const {cx,cy}=resolveBeatPosition(beat); const p=beat.props??{};
  const drawFn=p.draw?illustrations[p.draw]:null;
  if (!drawFn) return;
  ctx.save(); ctx.globalAlpha=state.alpha; ctx.translate(cx+state.offsetX,cy+state.offsetY); ctx.scale(state.scaleX,state.scaleY);
...
function renderFrame(ctx, t) {
  ctx.clearRect(0,0,W,H); drawBackground(ctx,t);
...
process.stderr.write(`[complete-chunk] Rendering frames ${startFrame}-${endFrame-1} to ${pngDir}\n`);
for (let frame = startFrame; frame < endFrame; frame++) {
  const t = frame / FPS;
  renderFrame(ctx, t);
  const buf = canvas.toBuffer("image/png");
  const fp = path.join(pngDir, "frame-" + String(frame).padStart(6, "0") + ".png");
  fs.writeFileSync(fp, buf);
  if (frame % 60 === 0) process.stderr.write(`  frame ${frame}/${endFrame}\n`);
}
process.stderr.write(`[complete-chunk] Done\n`);
